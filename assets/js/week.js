	...
	for (let day_index = 0; day_index < timetable.length; day_index++) {
		...
		for (period of timetable[day_index].schedule) {
			...
			if (period.subject == "" && period.room == "") {
				new_subject_container.innerHTML = `<span>+</span>` + `<span></span>`;
				...
				new_subject_container.innerHTML = `<span>${period.subject}</span>`.replace(/\r\n|\r|\n/g, "<br/>") + `<span>${period.room}</span>`.replace(/\r\n|\r|\n/g, "<br/>");

				new_subject_container.style.color = get_subject_color(period.subject);
			}

			new_subject_container.addEventListener("click", function (e) {
				var period_index;
