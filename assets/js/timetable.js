...
var refresh_periods_container = function () {
	...
	periods_container.addEventListener("click", function (e) {
		e.preventDefault();
	});
	...
	if (config.data.periods.length === 0) {
		periods_container.innerHTML = "<div class='no_period_placeholder' onclick='editor.show_period_edit_popup();'>+</div>";
	}

	for (period of config.data.periods) {
		periods_container.innerHTML += "<div class='period' onclick='editor.show_period_edit_popup();'>" + `<span>${period.start}</span>` + `<span>${period.end}</span>` + "</div>";
	}
};
...
function add_color_input_events() {
	...
	var color_input_preview = document.getElementsByClassName("color_preview")[0];

	color_input.addEventListener("input", function (e) {
		var color = e.target.value;

		color_input_preview.style.backgroundColor = `hsl(${color}, 100%, 50%)`;
	});
...
function random_color_input() {
    var randomColor = Math.floor(Math.random() * 360);
	var color_input = document.querySelector("[name='color_input']");
	var color_input_preview = document.getElementsByClassName("color_preview")[0];
	
    color_input.value = randomColor;
    color_input_preview.style.backgroundColor = `hsl(${randomColor}, 100%, 50%)`;
}
...
var editor = {
	...
	show_schedule_edit_popup: function (day_index, period_index) {
		...
		var message = config.data.timetable[day_index].schedule[period_index].message;
		var color = config.data.colors[subject];

		if (color === undefined || subject == "") {
			color = 0;
		}
		...
		document.querySelector("[name='color_input']").value = color;
		document.querySelector("[name='message_input']").value = message;
	...
	save_schedule_changes: function () {
		...
		var message = document.querySelector("[name='message_input']").value;
		
    	if (Array.isArray(config.data.colors)) {
    	    config.data.colors = {};
    	}
		...
		new_data.timetable[day_index].schedule[period_index].message = message;
...
function get_subject_color(subject) {
	...
	if (localStorage.getItem("schedule_dark") == "true" && printing_mode === false) {
